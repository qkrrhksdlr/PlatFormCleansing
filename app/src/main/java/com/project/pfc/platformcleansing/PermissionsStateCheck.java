...
public class PermissionsStateCheck {            // 권한 확인
    ...
    public boolean permissionState(Context context, int permission_index){                  //받아온 권한 상태 반환
        return PackageManager.PERMISSION_GRANTED == ContextCompat.checkSelfPermission(context,permissions[permission_index]);
    }

    public void setPermissions(Activity activity, int permisson_index, int requestCode){   //받아온 권한 승인 요청
        ActivityCompat.requestPermissions(activity, new String[] {permissions[permisson_index]}, requestCode);
    }

    public void setAllPermissions(Activity activity, int requestCode){
